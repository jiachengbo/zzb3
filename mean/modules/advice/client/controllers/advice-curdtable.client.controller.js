(function () {
  ...
  AdviceCURDTableController.$inject = ['$scope', 'Notification', '$log', '$window',
    ...
    '$uibModal', 'baseCodeService'];
  function AdviceCURDTableController($scope, Notification, $log, $window,
                                       uiGridConstants, AdviceService, $uibModal, baseCodeService) {
    var vm = this;
    //表数据
    vm.tableData = [];
    //ui-grid 当前选择的行
      ...
      modalInstance.result.then(function(result) {
        ...
        result.$save()
          .then(function(res) {
            ...
            Notification.success({ message: '<i class="glyphicon glyphicon-ok"></i> advice add saved successfully!' });
          })
          .catch(function(err) {
    ...
    vm.remove = function() {
      if ($window.confirm('Are you sure you want to remove selected record?')) {
        vm.selectedRow.$remove(function() {
          ...
          //复位当前行
          vm.selectedRow = null;
          Notification.success({ message: '<i class="glyphicon glyphicon-ok"></i> advice deleted successfully!' });
        })
        .catch(function(err) {
        ...
        if (isupdate) {
          result.$update()
            .then(function(res) {
              ...
              Notification.success({ message: '<i class="glyphicon glyphicon-ok"></i> advice update saved successfully!' });
            })
            .catch(function(err) {
              $log.error('advice update save error:', err.data.message);
              Notification.error({ message: err.data.message, title: '<i class="glyphicon glyphicon-remove"></i> ' +
              'advice update save error!' });
            });
        }
      }).catch(function(reason) {
        $log.log('Modal dismissed:', reason);
    ...
    //取后台Advice表所有数据
    AdviceService.query().$promise.then(function(data) {
