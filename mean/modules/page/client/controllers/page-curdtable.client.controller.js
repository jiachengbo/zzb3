(function () {
  ...
  angular
    .module('page')
    .controller('PageCURDTableController', PageCURDTableController);

  PageCURDTableController.$inject = ['$scope', 'Notification', '$log', '$window',
    'uiGridConstants', 'PageService',
    '$uibModal'];
  function PageCURDTableController($scope, Notification, $log, $window,
                                       uiGridConstants, PageService, $uibModal) {
    var vm = this;
    //表数据
    vm.tableData = [];
    //ui-grid 当前选择的行
    ...
    vm._openModal = function(resarg) {
      return $uibModal.open({
        templateUrl: '/modules/page/client/views/page-modal-form.client.view.html',
        controller: 'PageModalFormController',
        controllerAs: 'vm',
        backdrop: 'static',
        resolve: resarg
      });
      ...
      modalInstance.result.then(function(result) {
        ...
        result.$save()
          .then(function(res) {
            ...
            Notification.success({ message: '<i class="glyphicon glyphicon-ok"></i> page add saved successfully!' });
          })
          .catch(function(err) {
            $log.error('page add save error:', err.data.message);
    ...
    vm.remove = function() {
      if ($window.confirm('Are you sure you want to remove selected record?')) {
        ...
        .catch(function(err) {
          $log.error('page deleted error:', err.data.message);
          Notification.error({ message: err.data.message, title: '<i class="glyphicon glyphicon-remove"></i>' +
          ' page delete error!' });
        });
      }
        ...
        if (isupdate) {
          result.$update()
            .then(function(res) {
              ...
              Notification.success({ message: '<i class="glyphicon glyphicon-ok"></i> page update saved successfully!' });
            })
            .catch(function(err) {
              $log.error('page update save error:', err.data.message);
              Notification.error({ message: err.data.message, title: '<i class="glyphicon glyphicon-remove"></i> ' +
              'page update save error!' });
            });
        }
      }).catch(function(reason) {
        $log.log('Modal dismissed:', reason);
    ...
    //取后台Page表所有数据
    PageService.query().$promise.then(function(data) {
